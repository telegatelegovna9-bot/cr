      ...
      if (recent[i].low < recent[i - 1].low &&
        ...
        if (tests >= 2) {
          patterns.push({
            ...
            points: [
              { price: level, timestamp: recent[i].time },
            ],
    ...
    if (highs.length >= 2 && lows.length >= 2) {
      ...
      if (Math.abs(highSlope) < 0.001 && lowSlope > 0) {
        patterns.push({
          ...
          confidence: 0.7,
          points: [...highs, ...lows].map(h => ({ price: h.price, timestamp: recent[h.idx].time })),
    ...
    if (Math.abs(highReg.slope - lowReg.slope) / Math.abs(highReg.slope) < 0.3) {
      if (highReg.slope > 0.001) {
        patterns.push({
          ...
          confidence: 0.65,
          points: [
            { price: highReg.slope * 0 + highReg.intercept, timestamp: recent[0].time },
            { price: highReg.slope * (recent.length - 1) + highReg.intercept, timestamp: recent[recent.length - 1].time },
          ],
  ...
  private detectWedges(
    ...
    if (highReg.slope > 0 && lowReg.slope > 0 && highReg.slope < lowReg.slope) {
      patterns.push({
        ...
        confidence: 0.65,
        points: [
          { price: recent[0].high, timestamp: recent[0].time },
          { price: recent[recent.length - 1].high, timestamp: recent[recent.length - 1].time },
        ],
    ...
    if (highReg.slope < 0 && lowReg.slope < 0 && Math.abs(highReg.slope) > Math.abs(lowReg.slope)) {
      patterns.push({
        ...
        confidence: 0.65,
        points: [
          { price: recent[0].low, timestamp: recent[0].time },
          { price: recent[recent.length - 1].low, timestamp: recent[recent.length - 1].time },
        ],
  ...
  private detectBOS(
    ...
    if (last.close > maxHigh && maxIdx < recent.length - 3) {
      patterns.push({
        ...
        confidence: 0.75,
        points: [
          { price: maxHigh, timestamp: recent[maxIdx].time },
          { price: last.close, timestamp: last.time },
        ],
    ...
    if (last.close < minLow && minIdx < recent.length - 3) {
      patterns.push({
        ...
        confidence: 0.75,
        points: [
          { price: minLow, timestamp: recent[minIdx].time },
          { price: last.close, timestamp: last.time },
        ],
    ...
    if (highs.length >= 3) {
      ...
      if (last3[2] < last3[1] && last3[1] < last3[0]) {
        patterns.push({
          ...
          confidence: 0.65,
          points: last3.map((p, i) => ({ price: p, timestamp: recent[recent.length - 3 + i]?.time || Date.now() })),
    ...
    for (let i = 2; i < candles.length; i++) {
      ...
      if (c3.low > c1.high && c2.close > c2.open) {
        patterns.push({
          ...
          confidence: 0.7,
          points: [
            { price: c1.high, timestamp: c1.time },
            { price: c3.low, timestamp: c3.time },
          ],
      ...
      if (c3.high < c1.low && c2.close < c2.open) {
        patterns.push({
          ...
          confidence: 0.7,
          points: [
            { price: c1.low, timestamp: c1.time },
            { price: c3.high, timestamp: c3.time },
          ],
    ...
    for (let i = 1; i < recent.length - 1; i++) {
      ...
      if (curr.close < curr.open && next.close > next.open && next.close > curr.high) {
        patterns.push({
          ...
          confidence: 0.65,
          points: [
            { price: curr.low, timestamp: curr.time },
            { price: curr.high, timestamp: curr.time },
          ],
    ...
    for (let i = 5; i < recent.length; i++) {
      ...
      if (curr.high > maxHigh && curr.close < maxHigh) {
        patterns.push({
          ...
          confidence: 0.7,
          points: [
            { price: maxHigh, timestamp: window.find(c => c.high === maxHigh)?.time || curr.time },
            { price: curr.close, timestamp: curr.time },
          ],
      ...
      if (curr.low < minLow && curr.close > minLow) {
        patterns.push({
          ...
          confidence: 0.7,
          points: [
            { price: minLow, timestamp: window.find(c => c.low === minLow)?.time || curr.time },
            { price: curr.close, timestamp: curr.time },
          ],
